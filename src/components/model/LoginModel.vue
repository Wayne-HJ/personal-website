  ...
  methods: {
    ...
    login() {
      ...
      if (this.password.trim().length == 0) {
        this.$toast({ type: "error", message: "password can't be null" });
        return false;
      }
      const that = this;
        ...
        if (res.ret === 0) {
          ...
          that.axios.post("/api/login", param).then(({ data }) => {
            if (data.flag) {
              ...
              that.$toast({ type: "success", message: "login success" });
            } else {
